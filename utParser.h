...
TEST_F(ParserTest, ListAsStruct) {
    Scanner scanner(".(1,[])");
    Parser parser(scanner);
    Struct* testStruct = dynamic_cast<Struct *>(parser.createTerm());
...
TEST_F(ParserTest, parseStructNoArg) {
    ...
    EXPECT_EQ("point()", testStruct->symbol());
}
...
TEST_F(ParserTest, parseStructTwoArgs) {
    ...
    EXPECT_EQ("point(11, 12)", testStruct->symbol());
}
...
TEST_F(ParserTest, parseStructDOTSTwoArgs) {
    ...
    EXPECT_EQ("...(11, 12)", testStruct->symbol());
}
...
TEST_F(ParserTest, parseStructOneArg) {
    ...
    EXPECT_EQ("point(11)", testStruct->symbol());
}
